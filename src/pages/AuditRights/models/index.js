...
export default {
  ...
  effects: {
    ...
    *addUserOrRole({ payload, callback }, { call, put }) {
      const { code } = yield call(api.addUserOrRole, payload);
      callback(code);
    },

    // 更新用户或角色状态
    *updateUserOrRoleStatus({ payload, callback }, { call, put }) {
      const { code } = yield call(api.updateUserOrRoleStatus, payload);
    ...
    *updateUserOrRoleInfo({ payload, callback }, { call, put }) {
      ...
      const { code } = yield call(api.updateUserOrRoleInfo, params);
